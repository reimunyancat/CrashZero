...
import { snapPointToLinks } from '../utils/spatial.js';
...
function summarize(features) {
  ...
  for (const f of features) counts[f.risk_band] = (counts[f.risk_band] ?? 0) + 1;
  return counts;
}

function adjustRiskByBlackspots(segments, blackspots) {
  if (!blackspots.length) return segments;
  const links = segments.map((s) => ({ link_id: s.link_id, geometry: s.geometry }));
  // For each blackspot, find nearest link and boost risk.
  const boostMap = new Map();
  for (const bs of blackspots) {
    const snap = snapPointToLinks(bs.centroid[0], bs.centroid[1], links, 1500);
    if (!snap) continue;
    const influence = influenceFromMeters(snap.distance_m) * bs.source_weight;
  ...
  return segments.map((s) => {
    ...
    return { ...s, risk, risk_band: scoreToBand(risk), ead_baseline: risk * 0.18 };
  });
}
